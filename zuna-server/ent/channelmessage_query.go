...
func (_q *ChannelMessageQuery) QuerySender() *UserQuery {
...
func (_q *ChannelMessageQuery) FirstID(ctx context.Context) (id int64, err error) {
	var ids []int64
...
func (_q *ChannelMessageQuery) FirstIDX(ctx context.Context) int64 {
...
func (_q *ChannelMessageQuery) OnlyID(ctx context.Context) (id int64, err error) {
	var ids []int64
...
func (_q *ChannelMessageQuery) OnlyIDX(ctx context.Context) int64 {
...
func (_q *ChannelMessageQuery) IDs(ctx context.Context) (ids []int64, err error) {
...
func (_q *ChannelMessageQuery) IDsX(ctx context.Context) []int64 {
	ids, err := _q.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (_q *ChannelMessageQuery) WithSender(opts ...func(*UserQuery)) *ChannelMessageQuery {
	query := (&UserClient{config: _q.config}).Query()
...
//		GroupBy(channelmessage.FieldClientMessageID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.ChannelMessage.Query().
//		Select(channelmessage.FieldClientMessageID).
//		Scan(ctx, &v)
...
func (_q *ChannelMessageQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*ChannelMessage, error) {
	...
	if query := _q.withSender; query != nil {
		if err := _q.loadSender(ctx, query, nodes, nil,
			func(n *ChannelMessage, e *User) { n.Edges.Sender = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *ChannelMessageQuery) loadSender(ctx context.Context, query *UserQuery, nodes []*ChannelMessage, init func(*ChannelMessage), assign func(*ChannelMessage, *User)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*ChannelMessage)
	for i := range nodes {
		if nodes[i].user_channel_messages_sent == nil {
			continue
		}
		fk := *nodes[i].user_channel_messages_sent
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_channel_messages_sent" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *ChannelMessageQuery) loadChannel(ctx context.Context, query *ChannelQuery, nodes []*ChannelMessage, init func(*ChannelMessage), assign func(*ChannelMessage, *Channel)) error {
	...
	nodeids := make(map[string][]*ChannelMessage)
	for i := range nodes {
		if nodes[i].channel_channel_messages == nil {
			continue
		}
		fk := *nodes[i].channel_channel_messages
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "channel_channel_messages" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *ChannelMessageQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(channelmessage.Table, channelmessage.Columns, sqlgraph.NewFieldSpec(channelmessage.FieldID, field.TypeInt64))
	_spec.From = _q.sql
