...
import { encrypt, decrypt } from "../../crypto/x25519";
...
export function useMessages(
  ...
  const sendChatMessage = useCallback(
    async (
      ...
      const encryptedPreviewContent = encrypt(
  ...
  const uploadAndSend = useCallback(
    async (file: File, plaintext: string) => {
      ...
      try {
        ...
        // Encrypt file using shared crypto (X25519 DH + AES-GCM)
